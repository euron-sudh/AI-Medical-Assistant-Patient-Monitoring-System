...
interface MedicalHistoryEntry { id: string; condition_name: string; diagnosis_date: string|null; status: string; notes: string|null; }
...
export default function PatientDetailPage() {
  ...
  const fetchAll = async () => {
    ...
    try { const r = await apiClient.get(`/patients/${patientId}/medical-history`); setHistory(Array.isArray(r.data.history??r.data)?r.data.history??r.data:[]); } catch {}
    setLoading(false);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card shadow-sm">
        ...
        <div className="p-6">
          ...
          {activeTab==="history" && (history.length===0 ? <p className="py-8 text-center text-sm text-muted-foreground">No history entries.</p> : <div className="space-y-3">{history.map((h) => (<div key={h.id} className="rounded-lg border border-border p-4"><div className="flex items-center justify-between"><p className="font-medium text-foreground">{h.condition_name}</p><span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${h.status==="active"?"bg-amber-100 text-amber-800":"bg-green-100 text-green-800"}`}>{h.status}</span></div>{h.diagnosis_date && <p className="mt-1 text-xs text-muted-foreground">Diagnosed: {new Date(h.diagnosis_date).toLocaleDateString()}</p>}{h.notes && <p className="mt-1 text-xs text-muted-foreground">{h.notes}</p>}</div>))}</div>)}
        </div>
      </div>
    </div>
  );
